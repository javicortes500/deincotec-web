...
function validateAllPagesAfterLoad() {
    ...
    const page9HasData = document.getElementById('cond_1')?.checked ||
                        document.getElementById('cond_2')?.checked ||
                        document.getElementById('cond_3')?.checked ||
                        document.getElementById('cond_4')?.checked;
    if (page9HasData) {
...
function renderRecursosID(existing = []) {
    ...
    // Filas de la tabla
    const rows = [
        { label: 'Inmovilizado Material', id: 'inm_mat', readonly: true, bold: true },
        { label: 'Terrenos y edificios', id: 'terrenos', parent: 'inm_mat' },
        { label: 'Aparatos y equipos', id: 'aparatos', parent: 'inm_mat' },
        { label: 'TOTAL INMOVILIZADO MATERIAL', id: 'total_inm', readonly: true, bold: true, copyFrom: 'inm_mat' },
        { label: 'Inversiones Activos Fijos Materiales', id: 'inv_activos', readonly: true, bold: true },
        { label: 'Terrenos y edificios', id: 'inv_terrenos', parent: 'inv_activos' },
        { label: 'Aparatos y equipos', id: 'inv_aparatos', parent: 'inv_activos' },
        { label: 'Gastos Corrientes', id: 'gastos_corr', readonly: true, bold: true },
        { label: 'Personal', id: 'personal', parent: 'gastos_corr' },
        { label: 'Materiales', id: 'materiales', parent: 'gastos_corr' },
        { label: 'Colaboraciones externas y otros gastos', id: 'colaboraciones', parent: 'gastos_corr' },
        { label: 'TOTAL INVERSIONES GASTOS', id: 'total_inv_gastos', readonly: true, bold: true }
    ];
    
    rows.forEach(row => {
        const rowStyle = row.bold ? 'font-weight: 600; background-color: var(--md-sys-color-surface-container-low);' : '';
        html += `<tr><td style="border: 1px solid var(--md-sys-color-outline-variant); padding: 0.5rem; ${rowStyle}">${row.label}</td>`;
...
window.downloadClientExcel = async function(clientId) {
    ...
    try {
        ...
        const ws5Data = [
            ...
            ["Campo", "Valor"],
            ["Año", data.rec_ano || ''],
            ["Total Empleados", data.rec_total || ''],
            ["Hombres I+D", data.rec_id_hombres || ''],
            ["Mujeres I+D", data.rec_id_mujeres || ''],
            ["Año Inmovilizado", data.id_ano || ''],
            ["Inmovilizado Material", data.id_inmovilizado || ''],
            ["Gastos Corrientes", data.id_gastos_corrientes || '']
        ];
        ...
        const ws9Data = [
            ...
            ["Información Veraz", data.cond_3 ? 'Sí' : 'No'],
            ["CCAA", data.cond_4 ? 'Sí' : 'No']
        ];
