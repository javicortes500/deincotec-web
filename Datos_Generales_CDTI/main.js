...
function validateAllPagesAfterLoad() {
    ...
    // Página 5 - verificar si tiene datos
    const page5HasData = document.getElementById('id_ano')?.value?.trim() ||
                        document.getElementById('id_inmovilizado')?.value?.trim() ||
                        document.getElementById('id_gastos_corrientes')?.value?.trim();
    if (page5HasData) {
    ...
    // Página 9 - verificar si tiene datos (condiciones)
    validatePage9();
...
function renderRecursosID(existing = []) {
    ...
    const currentYear = now.getFullYear();
    const years = [];
    // Años históricos (3 anteriores) y previstos (2 siguientes) - igual que Recursos Humanos
    for (let y = currentYear - 3; y <= currentYear + 1; y++) years.push(y);
...
function calcularTotalesRecursosID(year) {
    // Inmovilizado Material = Terrenos + Aparatos
    ...
    // Total Inmovilizado Material = copia de Inmovilizado Material
    ...
    // Inversiones Activos Fijos = Terrenos inv + Aparatos inv
    ...
    // Gastos Corrientes = Personal + Materiales + Colaboraciones
    ...
    // Total Inversiones Gastos = Inversiones Activos Fijos + Gastos Corrientes
...
function initLoginSystem() {
    ...
    // Siempre configurar el formulario primero
    setupLoginForm();
    ...
    if (savedClientId) {
        ...
        authenticateClient(savedClientId).catch(error => {
            ...
            // Si falla la reconexión, limpiar localStorage y mostrar login
...
/** Configura el evento del formulario de login */
function setupLoginForm() {
    const loginForm = document.getElementById('login-form');
    if (loginForm) {
        console.log('✅ Formulario de login encontrado, configurando evento submit');
        // Clonar el formulario para eliminar todos los listeners anteriores
        const newLoginForm = loginForm.cloneNode(true);
        loginForm.parentNode.replaceChild(newLoginForm, loginForm);
        // Añadir el listener al formulario nuevo
...
function showLoginScreen() {
    ...
    // Mostrar advertencia si hay sesión guardada
    const savedClientId = localStorage.getItem('cdti-client-id');
    if (sessionWarning) {
...
/** Limpia la sesión guardada y recarga la página */
...
async function handleLogin(event) {
    ...
    try {
        // IMPORTANTE: Limpiar cualquier sesión anterior primero
        ...
        // Verificar si es contraseña de administrador
        if (password === ADMIN_KEY) {
            console.log('🔐 Contraseña de administrador detectada');
        ...
        // Validar contraseña contra colección de usuarios
        ...
        // Obtener el ID del cliente desde el documento del usuario
        ...
        // Asegurarse de limpiar el localStorage si falla
        localStorage.removeItem('cdti-client-id');
    } finally {
...
async function authenticateClient(clientId) {
    ...
    // Verificar si es la clave de administrador directa
    if (clientId === ADMIN_KEY) {
        console.log('🔑 Clave de administrador detectada, entrando en modo admin...');
        currentClientId = null;
        ...
        // Ir directamente al panel de administrador
        showMainApp();
        setView('admin');
        return;
    }
    ...
    // Mostrar/ocultar botón Admin según la contraseña
    const showAdminBtn = document.getElementById('show-admin-btn');
    if (showAdminBtn) {
...
function logout() {
    ...
    currentClientId = null;
    isAuthenticated = false;
    datosYaCargados = false; // Resetear bandera de carga
    ...
    // Ocultar ambas vistas (cliente y admin)
    const clientView = document.getElementById('client-view');
    const adminView = document.getElementById('admin-view');
...
function loadFormData(data) {
    ...
    Object.keys(data).forEach(key => {
        ...
        if (key === 'recursosHumanos' || key === 'recursosID' || key === 'productos' || key === 'accionarial' || 
    ...
    // Cargar recursos I+D si existen
    ...
    // Marcar que los datos ya fueron cargados para permitir auto-guardado
    datosYaCargados = true;
    console.log('✅ Datos cargados en el formulario - Auto-guardado habilitado');
    
    // Validar todas las páginas para actualizar los indicadores del menú
    setTimeout(() => {
...
async function loadClientData() {
    ...
    try {
        // Asegurarse de detener CUALQUIER listener anterior
        if (unsubscribe) {
            unsubscribe();
            unsubscribe = null;
        }
        ...
        unsubscribe = onSnapshot(clientDocRef, (docSnapshot) => {
            if (docSnapshot.exists()) {
                ...
                currentData = {};
                // Cliente nuevo - habilitar auto-guardado inmediatamente
                datosYaCargados = true;
...
function initializeMainApp() {
    ...
    // Validar todas las páginas para actualizar indicadores del menú
    setTimeout(() => {
        validateAllPagesAfterLoad();
    }, 200);
}
...
async function saveFormData(isFinalSave = false) {
    ...
    try {
        ...
        const formData = {
            ...
            cond_5: document.getElementById('cond_5').checked,
            cond_6: document.getElementById('cond_6').checked,
            cond_7: document.getElementById('cond_7').checked,
            cond_8: document.getElementById('cond_8').checked,
            cond_9: document.getElementById('cond_9').checked
        };
    ...
    unsubscribe = onSnapshot(q, (querySnapshot) => {
        ...
        querySnapshot.forEach((doc) => {
            ...
            const rowData = {
                ...
                Condicion_Gastos_Generales: data.cond_3 || false,
                Condicion_Cofinanciacion: data.cond_4 || false,
                Condicion_Confidencialidad: data.cond_5 || false,
                Condicion_DNSH: data.cond_6 || false,
                Condicion_Materias_Excluidas: data.cond_7 || false,
                Condicion_Veracidad: data.cond_8 || false,
                Condicion_CCAA: data.cond_9 || false,
...
window.downloadClientExcel = async function(clientId) {
    ...
    try {
        ...
        const ws9Data = [
            ["CONDICIONES"],
            [],
            ["Condiciones Presentación Empresa"],
            ["Condición", "Aceptada"],
            ["FEDER", data.cond_1 ? 'Sí' : 'No'],
            ["Tratamiento de Datos", data.cond_2 ? 'Sí' : 'No'],
            ["Gastos Generales (15%)", data.cond_3 ? 'Sí' : 'No'],
            ["Cofinanciación", data.cond_4 ? 'Sí' : 'No'],
            [],
            ["Condiciones Presentación Proyecto"],
            ["Condición", "Aceptada"],
            ["Confidencialidad y Transparencia", data.cond_5 ? 'Sí' : 'No'],
            ["DNSH (Reglamento ENAC)", data.cond_6 ? 'Sí' : 'No'],
            ["Materias Excluidas FEDER", data.cond_7 ? 'Sí' : 'No'],
            ["Declaración de Veracidad", data.cond_8 ? 'Sí' : 'No'],
            ["Condiciones CCAA", data.cond_9 ? 'Sí' : 'No']
        ];
...
/**
 * Valida la página 9 (Condiciones)
 * Retorna true si todas las condiciones están marcadas
 */
function validatePage9() {
    const conditionIds = ['cond_1', 'cond_2', 'cond_3', 'cond_4', 'cond_5', 'cond_6', 'cond_7', 'cond_8', 'cond_9'];
    
    const allChecked = conditionIds.every(id => document.getElementById(id)?.checked);
    const anyChecked = conditionIds.some(id => document.getElementById(id)?.checked);
    
    // Si alguno está marcado pero no todos -> error (rojo)
    // Si todos están marcados -> completado (verde)
    // Si ninguno está marcado -> sin error, sin completar
...
window.onload = function () {
        ...
        dataForm.addEventListener('input', (e) => {
            ...
            if (page3Section && !page3Section.classList.contains('hidden')) {
                ...
                if (e.target && e.target.id && e.target.id.includes('_pct')) {
                    // Usar debounce para evitar que se borre el valor mientras el usuario escribe
    ...
    // Añadir event listeners a las condiciones de la página 9
    const conditionIds = ['cond_1', 'cond_2', 'cond_3', 'cond_4', 'cond_5', 'cond_6', 'cond_7', 'cond_8', 'cond_9'];
