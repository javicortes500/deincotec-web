...
// Importar validador de NIF/CIF/NIE
import { validateNIFOrCIFOrNIE } from './nif-validator.js';
...
export function handleCPInputNotif(el) {
  if (!el) return;
  const cp = (el.value || '').replace(/\D/g, '').slice(0, 5);
  el.value = cp;
  const provinciaEl = document.getElementById('dirNotifProvincia');
  const localidadEl = document.getElementById('dirNotifLocalidad');
...
export function validatePage2() {
  ...
  if (viaEl) {
    ...
    if (!value) {
      ...
      errors.push('Desarrollo: Tipo de vía');
    } else {
      setFieldValid(viaEl);
    }
  }
  ...
  if (dirEl) {
    ...
    if (!value) {
      ...
      errors.push('Desarrollo: Dirección');
    } else {
      setFieldValid(dirEl);
    }
  }
  ...
  if (numEl) {
    ...
    if (!value) {
      ...
      errors.push('Desarrollo: Número');
    } else if (!/^\d+$/.test(value)) {
      setFieldError(numEl, 'Solo se permiten números');
      errors.push('Desarrollo: Número debe ser numérico');
    } else {
      setFieldValid(numEl);
    }
  }
  ...
  if (cpEl) {
    if (!cp) {
      ...
      errors.push('Desarrollo: Código Postal');
  ...
  if (provinciaEl) {
    ...
    if (!value) {
      ...
      errors.push('Desarrollo: Provincia');
    } else {
  ...
  if (localidadEl) {
    ...
    if (!value) {
      ...
      errors.push('Desarrollo: Localidad');
    } else {
  ...
  if (telEl) {
    ...
    if (!value) {
      ...
      errors.push('Desarrollo: Teléfono');
  ...
  const cpNotifEl = document.getElementById('dirNotifCP');
  const provinciaNotifEl = document.getElementById('dirNotifProvincia');
  const localidadNotifEl = document.getElementById('dirNotifLocalidad');
  const viaNotifEl = document.getElementById('dirNotifTipoVia');
  const dirNotifEl = document.getElementById('dirNotifDireccion');
  const numNotifEl = document.getElementById('dirNotifNumero');
  const telNotifEl = document.getElementById('dirNotifTelefono');
  const emailNotifEl = document.getElementById('dirNotifEmail');

  // Tipo de vía
  if (viaNotifEl) {
    const value = (viaNotifEl.value || '').trim();
    if (!value) {
      setFieldError(viaNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: Tipo de vía');
    } else {
      setFieldValid(viaNotifEl);
    }
  }

  // Dirección
  if (dirNotifEl) {
    const value = (dirNotifEl.value || '').trim();
    if (!value) {
      setFieldError(dirNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: Dirección');
    } else {
      setFieldValid(dirNotifEl);
    }
  }

  // Número
  if (numNotifEl) {
    const value = (numNotifEl.value || '').trim();
    if (!value) {
      setFieldError(numNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: Número');
    } else if (!/^\d+$/.test(value)) {
      setFieldError(numNotifEl, 'Solo se permiten números');
      errors.push('Notificaciones: Número debe ser numérico');
    } else {
      setFieldValid(numNotifEl);
    }
  }

  // CP
  const cpNotif = (cpNotifEl?.value || '').trim();
  if (cpNotifEl) {
    if (!cpNotif) {
      setFieldError(cpNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: Código Postal');
    } else if (!validSpanishCP(cpNotif)) {
      setFieldError(cpNotifEl, 'Código Postal inválido (5 dígitos)');
      errors.push('Notificaciones: Código Postal');
    } else {
      setFieldValid(cpNotifEl);
    }
  }

  // Provincia
  if (provinciaNotifEl) {
    const value = (provinciaNotifEl.value || '').trim();
    if (!value) {
      setFieldError(provinciaNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: Provincia');
    } else {
      setFieldValid(provinciaNotifEl);
    }
  }

  // Localidad
  if (localidadNotifEl) {
    const value = (localidadNotifEl.value || '').trim();
    if (!value) {
      setFieldError(localidadNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: Localidad');
    } else {
      setFieldValid(localidadNotifEl);
    }
  }

  // Teléfono
  if (telNotifEl) {
    const value = (telNotifEl.value || '').trim();
    if (!value) {
      setFieldError(telNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: Teléfono');
    } else if (!validatePhone9(value)) {
      setFieldError(telNotifEl, 'Teléfono debe tener 9 dígitos');
      errors.push('Notificaciones: Teléfono');
    } else {
      setFieldValid(telNotifEl);
    }
  }

  // Email
  if (emailNotifEl) {
    const value = (emailNotifEl.value || '').trim();
    if (!value) {
      setFieldError(emailNotifEl, 'Este campo es requerido');
      errors.push('Notificaciones: E-Mail');
    } else if (!validateEmail(value)) {
      setFieldError(emailNotifEl, 'Email inválido');
      errors.push('Notificaciones: E-Mail');
    } else {
      setFieldValid(emailNotifEl);
    }
  }
...
// validateField estilo Página 1: required / email / phone / nif / number
export function validateField(element, type) {
  ...
  switch (type) {
    ...
    case 'nif':
      ...
      const validation = validateNIFOrCIFOrNIE(value);
      isValid = validation.valid;
      errorMsg = validation.error || 'NIF/CIF/NIE inválido';
      break;
    case 'number':
...
export function validateNumericField(el) {
  ...
  // Filtrar caracteres no permitidos: solo números y coma
...
// Valida campos de números enteros (sin decimales)
export function validateIntegerField(el) {
  if (!el) return false;
  ...
  // Verificar que solo contenga números (sin punto ni coma)
  const hasInvalidChars = /[^0-9]/.test(v);

  if (hasInvalidChars) {
    setFieldError(el, 'Solo números enteros');
    return false;
  }
...
// Asegurar que las funciones estén disponibles globalmente para oninput inline
...
export function validatePercentageField(el) {
  ...
  if (v === '') {
    ...
    if (typeof window.debouncedSave === 'function' && window.datosYaCargados) {
...
export function validatePage3() {
  ...
  if (capitalSocialEl) {
    ...
    if (!value) {
      ...
      // Eliminar puntos de miles antes de validar
      const cleanValue = value.replace(/\./g, '');
      const hasInvalidChars = /[^0-9.,\-]/.test(cleanValue);
      if (hasInvalidChars) {
        setFieldError(capitalSocialEl, 'Solo se permiten números');
        errors.push('Capital Social debe ser numérico');
      } else {
        const numValue = Number(cleanValue.replace(',', '.'));
        if (Number.isNaN(numValue)) {
  ...
  // Validar grupos accionariales (TODOS los campos son obligatorios)
    ...
    grupos.forEach((grupo, idx) => {
      ...
      campos.forEach(el => {
        ...
        if (!val) {
          ...
          errors.push(`Accionista ${idx + 1}: ${el.id.split('_').pop()} es requerido`);
          grupoCompleto = false;
        } else {
          // Validaciones específicas
          if (el.id.includes('_cif')) {
            // Validar formato NIF/CIF si se desea, por ahora solo requerido
            // if(!validateField(el, 'nif')) { errors.push(...); grupoCompleto = false; }
            el.classList.add('input-valid');
          } else {
            el.classList.add('input-valid');
          }
  ...
  // Al menos un accionista es obligatorio
  if (accionarialesCompletos === 0 && (!accionarialContainer || accionarialContainer.children.length === 0)) {
    errors.push('Debe añadir al menos un Accionista');
  }

  // Validar Consejo de Administración (TODOS los campos son obligatorios)
  ...
  if (consejoContainer) {
    ...
    grupos.forEach((grupo, idx) => {
      ...
      campos.forEach(el => {
        ...
        if (!val) {
          el.classList.add('input-invalid');
          errors.push(`Consejo ${idx + 1}: ${el.id.split('_').pop()} es requerido`);
          grupoCompleto = false;
        } else {
          // Validación específica para NIF/CIF
          if (el.id.includes('_dni')) {
            // Usar la validación 'nif' que permite NIF y CIF
            if (!validateField(el, 'nif')) {
              // validateField ya pone la clase input-invalid y el tooltip
              errors.push(`Consejo ${idx + 1}: NIF/CIF inválido`);
              grupoCompleto = false;
            } else {
              // validateField ya pone input-valid
            }
          } else {
            el.classList.add('input-valid');
          }
      ...
      if (grupoCompleto) consejoCompletos++;
    });
  }

  if (consejoCompletos === 0 && (!consejoContainer || consejoContainer.children.length === 0)) {
    errors.push('Debe añadir al menos un Miembro del Consejo');
  }
...
export function validatePage7() {
  ...
  if (volumenAntEl) {
    ...
    if (rawVolAnt === '') {
      ...
      const cleanVolAnt = rawVolAnt.replace(/\./g, '');
      if (/[^0-9.,\-]/.test(cleanVolAnt)) {
        ...
        const numVolAnt = Number(cleanVolAnt.replace(',', '.'));
  ...
  if (balanceAntEl) {
    ...
    if (rawBalAnt === '') {
      ...
      const cleanBalAnt = rawBalAnt.replace(/\./g, '');
      if (/[^0-9.,\-]/.test(cleanBalAnt)) {
        ...
        const numBalAnt = Number(cleanBalAnt.replace(',', '.'));
...
/**
 * Valida y restringe el campo de teléfono a exactamente 9 dígitos numéricos
 */
...
/**
 * Valida y formatea campos monetarios con separador de miles automático
 * Formato: 50.000,00 (punto para miles, coma para decimales)
 */
export function validateCurrencyField(input) {
  if (!input) return;
  ...
  // Obtener el valor actual y limpiar todo excepto números y coma
  let value = input.value;

  // Separar parte entera y decimal
  let parts = value.split(',');
  let integerPart = parts[0].replace(/[^0-9]/g, ''); // Solo números en parte entera
  let decimalPart = parts[1] ? parts[1].replace(/[^0-9]/g, '') : ''; // Sin límite de decimales

  // Si no hay números, limpiar y salir
