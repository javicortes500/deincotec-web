...
export function validatePage3(){
  ...
  if(accionarialContainer){
    ...
    // Validar que la suma de participaciones sea 100%
    if(accionarialesCompletos > 0) {
      sumaParticipaciones = Math.round(sumaParticipaciones * 10) / 10; // Redondear a 1 decimal
      
      // Marcar todos los campos de % como inválidos si la suma no es 100
      if(sumaParticipaciones !== 100) {
        grupos.forEach(grupo => {
          const pctEl = grupo.querySelector('[id*="_pct"]');
          if(pctEl && pctEl.value) {
            setFieldError(pctEl, `La suma debe ser 100% (Actual: ${sumaParticipaciones}%)`);
          }
        });
        errors.push(`Suma de participaciones debe ser 100% (Actual: ${sumaParticipaciones}%)`);
      } else {
        // Si suma 100, marcar como válidos
        grupos.forEach(grupo => {
          const pctEl = grupo.querySelector('[id*="_pct"]');
          if(pctEl && pctEl.value) {
            setFieldValid(pctEl);
